...
void* worker_sum(void* arg)
{
	...
	int startpart = (MAX_NO_OF_ELEMENTS / MAX_NO_OF_THREADS) * (current_thread_data->thread_number - 1);
	int endpart = (MAX_NO_OF_ELEMENTS / MAX_NO_OF_THREADS) * current_thread_data->thread_number;
...
int main()
{
	...
	for (int i = thread_no; i <= MAX_NO_OF_THREADS; i++) {
		arg_arr[i - 1].thread_number = i;
	}
